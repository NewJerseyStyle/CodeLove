...
label cee_C02_start:
    ...
    rusty "這些箱子明明已經沒用了，但你還在標籤上寫著『佔用中』，別人當然沒辦法用啊。"
    
    cee "（看著標籤）沒有接到釋放指令。手動管理模式。"
    
    cee "（試圖把一個新貨件塞進去，結果撞到了舊箱子）"
    ...
    narrator "你需要幫 Cee 建立一套清理機制：借用（分配）與 歸還（釋放）。"

    menu:
        "建議使用『借用證』制度 (malloc/free)":
